...
# This dictionary is the shared, in-memory session storage for vector databases.
# It's imported by the coordinator to add new vector stores.
...
async def retrieve_context(request: RetrievalRequest):
    ...
    db = session_vector_stores.get(request.session_id)
    if not db:
