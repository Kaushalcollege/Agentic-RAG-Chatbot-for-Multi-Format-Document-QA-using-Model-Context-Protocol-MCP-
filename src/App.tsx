...
export default function App() {
  ...
  const handleFileUpload = useCallback((file: File) => {
    ...
    const uploadAndProcess = async () => {
      try {
        ...
        // Calls the coordinator to start the session, using a relative URL for deployment
        const res = await fetch("/agent/coordinator/start_session", {
          method: "POST",
          body: formData,
        });
        if (!res.ok) throw new Error("File processing failed on the backend.");

        const data = await res.json();
        // Unpacks the MCP message to get the session details from the payload
  ...
  const handleSendMessage = useCallback(
    async (messageContent: string) => {
      ...
      try {
        // Formats the chat history for the backend
        const chatHistoryForApi = updatedMessages.slice(0, -1).map((msg) => ({
          role: msg.type === "user" ? "user" : "assistant",
          content: msg.content,
        }));

        // Calls the coordinator to ask a question, sending the session_id and history
        const res = await fetch("/agent/coordinator/query", {
          method: "POST",
        ...
        const data = await res.json(); // Gets the direct JSON response
  ...
  return (
    <div className="h-screen bg-background flex gap-6 p-6">
      {/* This structure provides the definitive fix for the scrolling layout */}
